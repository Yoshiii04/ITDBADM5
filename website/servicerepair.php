...
<head>
  ...
  <title>Admin - Service Repairs</title>
                ...
                <td>
                  <div class="d-flex align-items-center">
                    <?php if ($repair['product_image']): ?>
                      <img src="<?= htmlspecialchars($repair['product_image']) ?>" alt="<?= htmlspecialchars($repair['product_name']) ?>" class="repair-img">
                    <?php endif; ?>
                    <span><?= htmlspecialchars($repair['product_name'] ?? 'N/A') ?></span>
                  </div>
                </td>
                <td class="repair-description" title="<?= htmlspecialchars($repair['description']) ?>">
                  <?= htmlspecialchars($repair['description']) ?>
                </td>
                <td>
                  <?= htmlspecialchars($repair['username']) ?><br>
                  <small><?= htmlspecialchars($repair['email']) ?></small>
                </td>
                <td><?= htmlspecialchars($repair['request_date']) ?></td>
                <td>
                  <span class="badge 
                    <?= $repair['status'] == 'pending' ? 'badge-warning' : 
                       ($repair['status'] == 'in_progress' ? 'badge-info' : 'badge-success') ?>">
                    <?= htmlspecialchars(ucfirst(str_replace('_', ' ', $repair['status']))) ?>
                  </span>
                </td>
                <td>
