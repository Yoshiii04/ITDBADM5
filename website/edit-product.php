...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $image_path = $product['image']; // Retain existing image
    ...
    if (empty($product_name)) {
        ...
        if ($count > 0) {
            ...
            if (!$error_message) {
                ...
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("ssdiiis", $product_name, $description, $price, $category_id, $stock, $image_path, $product_id);
                if ($stmt->execute()) {
  ...
  <div class="dashboard-container">
    ...
    <main class="main-content">
      ...
      <form class="product-form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?id=' . $product_id; ?>" enctype="multipart/form-data">
        ...
        <input type="number" id="product-stock" name="product-stock" value="<?php echo htmlspecialchars($product['stock']); ?>" min="0" required>
        ...
        <?php if ($product['image'] && file_exists($product['image'])): ?>
          <p>Current Image: <img src="<?php echo htmlspecialchars($product['image']); ?>" alt="Current Image" width="50"></p>
        <?php else: ?>
